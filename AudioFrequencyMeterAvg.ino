// AudioFrequencyMeterAvg.ino
//
//  Extension of AudioFrequencyMeter.ino in which a moving avarage of the detected sound period is calculated 
//  before reporting the frequency.
//  The window size can be set with the variable N (by default an avg over 8 periods is taken)
//
...
unsigned long NperiodsTicks;  // keep track of total time in N periods: avg period = NperiodsTicks / N
...
ISR(ANALOG_COMP_vect) {
  if (threshState == HIGH) {  // detecting a rising edge of sound wave passing the threshold
    ...
    uint16_t curTCNT = TCNT1;  // store current timer state as soon as possible
    TCNT1 = 2;                 // reset counter, start counting from 2 to account for some systematic offset found in the calibration.

    if (lookForFirstCrossing) { // this is the first crossing, no period can be computed yet
      lookForFirstCrossing = false;
    } 
    else { // fill/update the buffer and compute the average period
      if (bufCount < N) { // the buffer is filling up for the first time

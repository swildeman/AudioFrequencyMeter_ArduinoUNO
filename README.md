...
The default configuration detects frequencies between 16Hz - 24kHz within 0.1% accuracy (if the tone is pure). The frequency range and resolution can be configured (esp. useful if lower frequencies need to be detected, as for higher frequencies code execution time starts to become a bottleneck).
